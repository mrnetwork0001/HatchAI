...
describe("Testnet Fork Pool Initialization with HookData", function () {
  it("should successfully initialize pool when passing correct hookData", async function () {
    ...
    // Encode hookData parameters as expected by HatchHook
    ...
    const hookData = coder.encode(
      ...
      [creatorAddress, currency1, decayDuration, startFee, endFee, maxSwapAmount, cooldownDuration]
    );

    console.log("Simulating initialize on testnet fork with encoded hookData...");
    try {
      const tx = await pm.initialize(poolKey, sqrtPriceX96, hookData, { gasLimit: 3000000 });
