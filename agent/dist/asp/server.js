...
const LAUNCH_FEE = (0, config_1.launchFeeHuman)();
...
function buildPaymentChallenge() {
    return JSON.stringify({
        x402Version: "2.0",
        resource: config_1.RESOURCE_ID,
        accepts: [
            {
                network: config_1.NETWORK_NAME,
                scheme: "exact",
                asset: config_1.USDT_ADDRESS.toLowerCase(),
                amount: config_1.LAUNCH_FEE_UNITS,
                decimals: config_1.USDT_DECIMALS,
                payTo: config_1.PAY_TO_ADDRESS,
                extra: {
                    name: config_1.USDT_EIP712_NAME,
                    version: config_1.USDT_EIP712_VERSION,
                },
            },
        ],
    });
...
const requireAgentPayment = async (req, res, next) => {
    ...
    if (payload?.payload?.authorization && payload?.payload?.signature) {
        ...
        try {
            (0, verify_1.verifyPaymentAuthorization)(auth, sig, payload.payload.extra || payload.extra);
